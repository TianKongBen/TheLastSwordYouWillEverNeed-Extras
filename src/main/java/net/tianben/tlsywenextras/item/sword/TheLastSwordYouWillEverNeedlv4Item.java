...
import net.tianben.tlsywen.entity.LDlv4Entity;
import org.jetbrains.annotations.Nullable;

import java.util.List;

public class TheLastSwordYouWillEverNeedlv4Item extends SwordItem {

    private final int attackDamage;

    private static final float ATTACK_SPEED = -2.4f;

    public TheLastSwordYouWillEverNeedlv4Item(ToolMaterial toolMaterial) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            LDlv4Entity lightningProjectileEntity = new LDlv4Entity(world, user);
