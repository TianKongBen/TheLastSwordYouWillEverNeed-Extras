...
import net.tianben.tlsywen.entity.LDlv7Entity;
import org.jetbrains.annotations.Nullable;

import java.util.List;

public class TheLastSwordYouWillEverNeedlv7Item extends SwordItem {

    private final int attackDamage;

    private static final float ATTACK_SPEED = -2.4f;

    public TheLastSwordYouWillEverNeedlv7Item(ToolMaterial toolMaterial) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            LDlv7Entity lightningProjectileEntity = new LDlv7Entity(world, user);
