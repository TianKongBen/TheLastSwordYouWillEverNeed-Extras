...
import net.tianben.tlsywen.entity.LDlv8Entity;
import org.jetbrains.annotations.Nullable;

import java.util.List;

public class TheLastSwordYouWillEverNeedlv8Item extends SwordItem {

    private final int attackDamage;

    private static final float ATTACK_SPEED = -2.4f;

    public TheLastSwordYouWillEverNeedlv8Item(ToolMaterial toolMaterial) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            LDlv8Entity lightningProjectileEntity = new LDlv8Entity(world, user);
